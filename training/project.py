...
class Project:
    ...
    def task_pivot_to_table_r(self):
        table = self.df.pivot_table(
            index="category", values="real_time", aggfunc=np.sum
        )
        table["(HH:MM)"] = table["real_time"].apply(
    ...
    def task_pivot_to_table_e(self):
        table = self.df.pivot_table(
            index="category", values="estimate_time", aggfunc=np.sum
        )
        table["(HH:MM)"] = table["estimate_time"].apply(
            lambda x: f"{int(x):02d}:{int(x % 1 * 60):02d}"
        )
        print("------> Total par catégorie")
        print(table)
    ...
    def task_pivot_to_pie(self):
        ...
        table = df[["category", "real_time"]].pivot_table(
            index="category", values="real_time", aggfunc=np.sum
        )
    ...
    def task_pivot_to_bar(self):
        df = pd.DataFrame([task.__dict__ for task in self.tasks])
        table1 = df[["category", "real_time"]].pivot_table(
            index="category", values="real_time", aggfunc=np.sum
        )
